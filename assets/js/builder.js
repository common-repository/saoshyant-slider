jQuery(function($) {
	...
	************************************************/
	...
	function auto_width_builder_warp(){
  				$('.sao-aw').each(function() {
			...
			widths =  auto_width_flex(widths,$(this),'col')  - 40;
	  		...
	  		if (900 < widths ){
				$(this).addClass('sao-1200');
			
 
			}else if (600 < widths && widths <= 900){
				$(this).addClass('sao-900');
						
			}else if (400 < widths && widths <= 600){
				$(this).addClass('sao-600');
							
			}else if (300 < widths && widths <= 400){
				$(this).addClass('sao-400');
	 
			} else if (200 < widths && widths <= 300){
				$(this).addClass('sao-300');
				
		 
 
			}else if ( 200 > widths ){
				$(this).addClass('sao-200');
 	...
 	function sao_fold_hide(items){ 
 		...
 		$('.sao_options').find('.sao_options_fold').each(function() {
			...
			$(this).find('.sao_options_fold_item').each(function() {
				...
				var type = $('.sao_options [name="'+data_name+'"]').attr('type');
				if(type == 'radio'){
					var checked = $('.sao_options [name="'+data_name+'"][value="'+data_value+'"]').attr('checked');
					if( checked == 'checked' ){
						show = 'checked';
					}
				}else{
					var val =$('.sao_options [name="'+data_name+'"]').val();
						if(data_value == val){
						show = 'checked';
					}
				}
		...
		$('.sao_options_fold_item').each(function() {
			var data_name = $(this).attr('data-name');
			var actives  = $('.sao_options [name="'+data_name+'"]').parents(items).attr('data-active');
			if(	 actives == 'hide' ){
			...
			$(document).on("click",'.sao_options_select .sao_options_setting, .sao_options_checkbox .sao_options_setting,.sao_options_radio .sao_options_setting,.sao_options_radio_image .sao_options_setting',function() { 
  					$(this).parents('.sao_options_warp').find('.sao_options_fold').each(function() {
					var show;
					$(this).find('.sao_options_fold_item').each(function() {
					var data_name = $(this).attr('data-name'); 			
					var data_value = $(this).attr('data-value');
			...
			$(this).parents('.sao_options_warp').find('.sao_options_fold_item').each(function() {
				var data_name = $(this).attr('data-name');
		...
		$(document).on("click",'.sao_title_tabs a',function() {
			var id =$(this).attr('data-id'); 
  					$('.sao_options_warp[data-tab="'+id+'"]').find('.sao_options_fold').each(function() {
					var show;
					$(this).find('.sao_options_fold_item').each(function() {
					var data_name = $(this).attr('data-name'); 			
					var data_value = $(this).attr('data-value');
			...
			$('.sao_options_warp[data-tab="'+id+'"]').find('.sao_options_fold_item').each(function() {
				var data_name = $(this).attr('data-name');
				var actives  = $('[name="'+data_name+'"]').parents(items).attr('data-active');
				if(	 actives == 'hide' ){
					$(this).parent().parent().attr('data-active','hide');
	...
	************************************************/
  	function sao_sortable_tabs(){ 
		...
		$('.sao_options_list .sao_options_setting ').sortable({
			opacity: 0.6,
			handle: ".sao_list_options_item_title",
			stop: function() {}
			}); 	
			...
			if( !preInitSaved ) {
				...
				delete preInitSaved.qtInit[initID_editor];
					...
					if ( typeof quicktags !== 'undefined' ) {
						...
						QTags( {'id': id_name } );                		QTags._buttonsInit();
					}
	...
	***************************************************/
	...
	$('.sao_builder_main .sao_model_element').on('click', '.sao_model_add', function(e) {
		...
		var data_value = $('.sao_model_element').find('.selected').attr('data-element');
		 var sao_element_options_json = $('.sao_model_element').find('.selected').attr('data-option');
 		$.ajax({
			type: 'POST',
			...
			data : {
				...
				value :  data_value,
				sao_element_options :  sao_element_options_json,
				default :  '1',
 			},
			success:function(data) {
 				if( data.length){
	...
	//Elements Options
 	$('.sao_builder_main').on('click','.sao_element_options', function() {
		...
		var data_this = $(this);
		 var sao_element_options_json = $('.sao_model_item[data-element="'+data_value+'"]').attr('data-option');
		...
		$('body').addClass('sao-body-panel');	
 		...
 		imageFrame.on( 'select', function() {
			...
			var id = attachment.attributes.id;
				...
				data+='<input  type="hidden" class="sao_attachment_'+name+'" name="'+ name+'" 	value="'+id+'">';		 
 				data+='<span class="sao_image_remove"></span><img  src="'+medium_url+'"/>';
				data+='</div>';
			$('body').removeClass('sao-body-panel');
...
$.ajax({
			type: 'POST',
 			...
 			url: builder_js.ajaxurl,
			data : {
				action : 'sao_icon_fonts',
				ajax : '1',
 			},
			success:function(data) {
 				if( data.length){
					$('.sao-icon-fonts').remove();
  					jQuery('body').append(data); 
 				}else{
					 remove_add_error_loading();
				} 
	...
	jQuery(document).on("keyup",".search-icon-control",function(){
		var val = $(this).val();
		if(val !== ''){
			 $(this).parents('.sao_icon').attr('sao-has-search','active');
 		}else{
			 $(this).parents('.sao_icon').attr('sao-has-search','deactive');
		}
	  $('.sao_icon_item').each(function(){
			 $(this).addClass('sao-search-item');
			if($(this).find('span').text().toLowerCase().indexOf(""+val+"") != -1 ){
			 $(this).addClass('sao-search-show');
			}else{
			 $(this).removeClass('sao-search-show');
			}
 	 });
	  $('.sao_icon_head').each(function(){
			 $(this).addClass('sao-search-item');
 	 });
	...
	*****************************************************/	
	jQuery(document).on("click" ,'.sao_title_tabs a' ,function(){
		...
		$(this).parents('.sao_icon,.sao_options_middle,.sao_model_middle').find('.sao_layout_group_active').removeClass('sao_layout_group_active');
		$(this).parents('.sao_icon,.sao_options_middle,.sao_model_middle').find('[data-tab="'+value+'"]').addClass('sao_layout_group_active');
	...
	*****************************************************/	
	...
	$(document).on('change keyup mousedown',".sao_list_item",function() { 
			var val = $(this).find('input,textarea').val();       
			$(this).find('.sao_title_list').html('').html(val);                
	});		
			  
	 /****************************************************
		sao_options_according-start
		*****************************************************/
		$(document).on('click', '.sao_options_according-start[data-according="hide"] span', function(e) {
			$('.sao_options_according-start').each( function(index, element) {
			$(this).attr('data-according','hide');
				});
			$(this).parent().attr('data-according','show');
		
		}); 
		$(document).on('click', '.sao_options_according-start[data-according="show"] span', function(e) {
		 
			$(this).parent().attr('data-according','hide');
	...
	*****************************************************/	
	...
	$(window).on('load',function(){
		sao_slider_item();
 	//	sao_model_element_load();
	});
	/*
	 setTimeout(function(){ 
	 		if($('.sao_builder_main').hasClass('sao_builder_main_has_loading') && $('.sao_builder_main').hasClass('sao_builder_main_loading')){
	  			sao_model_element_load();
			}
 	  }, 5000);
	*/
